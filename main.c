...
//lease kay�tlar�n� temsil eden struct
struct LeaseRecord {
   	char ip[16];
    char mac[18];
   	char startTime[21];
...
//strtok fonksiyonu ile par�alama i�lemi 
...
// Dosyan�n sonuna gidildi(fseek)
if (fseek(file, 0, SEEK_END) != 0) {
	perror("Dosya konumu degistirme hatasi!");
	fclose(file);
	return 1;
}  
// Dosya i�aret�isinin �u anki konumu al�nd�(ftell)
long dosyaBoyutu = ftell(file);
//-1 ya da -1L hatay� belirtiyor.l longtan geliyor
if (dosyaBoyutu == -1L) { 
...
// Dosya verilerini tutmak i�in dinamik bellek tahsisi yap�ld�
char *veriler = (char *) calloc(dosyaBoyutu + 1, sizeof(char)); //�nce 0 lar sonra tutar 
//  "abc"  'a' 'b' 'c' '\0'
if (veriler == NULL) {
